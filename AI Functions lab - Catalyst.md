...
> **Note:** The environment has been set up for you. The Snowflake role, warehouse, database, schema, stage, and PDF files are all ready to use.
...
## Step 1: Generate the Extraction Query with Cortex Code
...
1. Go to **Workspaces**, create a new SQL file called products.sql (+ Add new -> SQL file).
...
> **Note:** This query will return results for 4 of the 5 documents. The Dupixent monograph (147 pages) exceeds AI_EXTRACT's 125-page limit. The `AI_PARSE_DOCUMENT` + `SNOWFLAKE.CORTEX.COMPLETE` approach shown in Step 2 handles documents of any size by processing only the relevant pages.
